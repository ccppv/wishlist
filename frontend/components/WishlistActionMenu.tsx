...
export default function WishlistActionMenu({ wishlistId, shareToken, onEdit, onDelete }: WishlistActionMenuProps) {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 top-full mt-1 w-32 bg-white rounded-lg shadow-lg border border-gray-200 py-0.5 z-50">
          <button
            onClick={(e) => {
              e.preventDefault()
              e.stopPropagation()
            ...
            className="w-full text-left px-2 py-1.5 hover:bg-gray-50 transition flex items-center gap-1.5"
          >
            ...
            <span className="text-gray-900 text-xs">{copied ? '✓ Скопировано!' : 'Поделиться'}</span>
          </button>
          ...
          >
            ...
            <span className="text-gray-900 text-xs">Редактировать</span>
          </button>
          ...
          <button
            ...
            className="w-full text-left px-2 py-1.5 hover:bg-red-50 transition flex items-center gap-1.5"
          >
