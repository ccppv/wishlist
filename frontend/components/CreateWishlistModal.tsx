    ...
    <>
        ...
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          <div className="p-5">
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-lg font-semibold text-neutral-900">Создать вишлист</h2>
              <button onClick={handleClose} className="p-1 text-neutral-400 hover:text-neutral-600 rounded-lg transition">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="flex flex-col items-center relative" ref={coverRef}>
                <div
                  ...
                  className="w-20 h-20 rounded-xl overflow-hidden cursor-pointer border-2 border-gray-200 hover:border-primary-500 transition flex items-center justify-center bg-gray-50"
                >
                  {coverType === 'emoji' ? (
                ...
                {showCoverMenu && (
                  <div className="absolute top-24 z-20 bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden w-36">
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="text-gray-900">Эмодзи</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="text-gray-900">Фото</span>
                    </button>
                  </div>
                )}
              </div>
              ...
              {/* Description */}
              <div>
                <label htmlFor="desc" className="block text-sm font-medium text-gray-700 mb-1">Описание</label>
                <textarea
                  id="desc" rows={2}
                  value={formData.description}
                  onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none text-gray-900 resize-none"
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none text-gray-900 bg-white appearance-none"
              ...
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={handleClose}
                  className="flex-1 px-4 py-2.5 border border-neutral-300 text-neutral-700 font-medium rounded-xl hover:bg-neutral-50 transition">
                  Отмена
                </button>
      ...
      {showEmojiPicker && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
          ...
          <div className="relative bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl shadow-2xl max-h-[70vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom">
            {/* Header */}
            <div className="px-4 pt-4 pb-2 border-b border-gray-100">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-bold text-gray-900">Эмодзи</h3>
              ...
              <div className="relative mb-2">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-gray-100 rounded-lg text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500"
                />
              </div>
              {/* Category icons row */}
              {!emojiSearch && (
                <div className="flex gap-1 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                  {EMOJI_CATEGORIES.map((cat, i) => (
                    <button key={i} type="button" onClick={() => setEmojiCategory(i)}
                      className={`p-2 text-xl rounded-lg transition flex-shrink-0 ${emojiCategory === i ? 'bg-primary-100' : 'hover:bg-gray-100'}`}
                      title={cat.name}>
                      {cat.icon}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-8 gap-0.5">
                {filteredEmojis.map((emoji, idx) => (
                  <button key={`${emoji}-${idx}`} type="button" onClick={() => handleEmojiSelect(emoji)}
                    className={`p-2 text-2xl rounded-xl transition-all hover:scale-110 hover:bg-gray-100 ${
                      selectedEmoji === emoji && coverType === 'emoji' ? 'bg-primary-100 scale-110 ring-2 ring-primary-500' : ''
                    }`}>
                    {emoji}
                  </button>
                ))}
