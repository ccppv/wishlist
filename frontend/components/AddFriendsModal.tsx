...
export default function AddFriendsModal({ isOpen, onClose, username }: AddFriendsModalProps) {
  ...
  const getFriendButtonContent = (userId: number) => {
    ...
    switch (friendStatus.status) {
      case 'accepted':
        return { text: 'Друзья', className: 'bg-green-100 text-green-700 cursor-default', disabled: true }
      case 'pending':
        return { text: 'Заявка отправлена', className: 'bg-yellow-100 text-yellow-700 cursor-default', disabled: true }
      case 'self':
        return { text: 'Это вы', className: 'bg-gray-200 text-gray-500 cursor-default', disabled: true }
      default:
        return { text: 'Добавить', className: 'bg-primary-600 hover:bg-primary-700 text-white', disabled: false }
    }
  }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="p-5">
          ...
          <div className="flex justify-between items-center mb-5">
            <h2 className="text-lg font-semibold text-neutral-900">Добавить друзей</h2>
          ...
          <div className="mb-6">
            ...
            <form onSubmit={handleSearch} className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition text-gray-900"
              />
              <button
                type="submit"
              ...
              <div className="mt-4 space-y-2">
                {searchResults.map((user) => {
                  ...
                  return (
                    <div
                      key={user.id}
                      className="w-full p-3 flex items-center gap-3 hover:bg-gray-50 rounded-lg transition"
                    >
                      <button
                      ...
                      >
                        <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0">
                          {user.avatar_url ? (
                            ...
                            <span className="text-gray-600 font-medium">{user.username[0].toUpperCase()}</span>
                          )}
                        </div>
            ...
            {searchQuery && !isSearching && searchResults.length === 0 && (
              <p className="mt-4 text-sm text-gray-500 text-center">Пользователи не найдены</p>
            )}
          </div>
          ...
          <div>
            ...
            {copied && (
              <p className="text-sm text-green-600 mt-2">Ссылка скопирована</p>
            )}
          </div>

          {/* Info */}
          <div className="mt-5 p-3.5 bg-neutral-50 rounded-xl">
