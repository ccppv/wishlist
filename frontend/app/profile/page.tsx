    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-30 px-3 pt-3">
        <div className="bg-header border border-gray-200/40 rounded-2xl shadow-sm">
          <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center">
            <Link href="/" className="text-lg font-bold tracking-tight text-neutral-900 hover:opacity-80 transition">
              Wishlist
            </Link>

            {/* Desktop nav */}
            <nav className="hidden sm:flex items-center gap-1">
              <Link href="/gifts" className="p-2 text-neutral-400 hover:text-neutral-700 rounded-lg transition" title="Подарки друзьям">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" />
                </svg>
              </Link>
              <Link href="/settings" className="p-2 text-neutral-400 hover:text-neutral-700 rounded-lg transition" title="Настройки">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
              </Link>
              <button onClick={() => setShowAddFriendsModal(true)} className="p-2 text-neutral-400 hover:text-neutral-700 rounded-lg transition" title="Найти друга">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
              ...
              <div className="relative">
                <button onClick={() => setShowFriendRequestsModal(!showFriendRequestsModal)}
                  className="p-2 text-neutral-400 hover:text-neutral-700 rounded-lg transition relative" title="Уведомления">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
              ...
              <button onClick={() => setShowQRCodeModal(true)} className="p-2 text-neutral-400 hover:text-neutral-700 rounded-lg transition" title="QR-код">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" />
                </svg>
              </button>
              <div className="w-px h-5 bg-neutral-200 mx-1" />
              <button onClick={handleLogout} className="text-neutral-400 hover:text-neutral-700 text-sm transition">
                Выйти
              </button>
            </nav>
            ...
            <div className="flex sm:hidden items-center gap-0.5">
              <div className="relative">
                <button onClick={() => setShowFriendRequestsModal(!showFriendRequestsModal)}
                  className="p-2 text-neutral-400 hover:text-neutral-700 rounded-lg transition relative">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                  </svg>
                  {friendRequestsCount > 0 && (
              ...
              <button onClick={() => setShowQRCodeModal(true)} className="p-2 text-neutral-400 hover:text-neutral-700 rounded-lg transition">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" />
                </svg>
              </button>
              <button onClick={() => setShowBurger(true)} className="p-2 text-neutral-400 hover:text-neutral-700 rounded-lg transition">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
      ...
      {showBurger && (
        <div className="fixed inset-0 z-50 sm:hidden">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-72 bg-white/95 backdrop-blur-xl shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-4">
              <span className="font-semibold text-neutral-900">Меню</span>
              <button onClick={() => setShowBurger(false)} className="p-1 text-neutral-400 hover:text-neutral-700">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
            ...
            <nav className="flex-1 py-1">
              {[
                ...
                <Link key={item.href} href={item.href} onClick={() => setShowBurger(false)}
                  className="flex items-center gap-3 px-5 py-3.5 text-neutral-600 hover:bg-neutral-50 transition">
                  <svg className="w-5 h-5 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.6}>
              ...
              <button onClick={() => { setShowBurger(false); setShowAddFriendsModal(true) }}
                className="w-full flex items-center gap-3 px-5 py-3.5 text-neutral-600 hover:bg-neutral-50 transition">
                <svg className="w-5 h-5 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.6}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
                </svg>
            ...
            <div className="border-t border-neutral-100 px-5 py-4">
              <button onClick={() => { setShowBurger(false); handleLogout() }}
                className="w-full py-2.5 text-red-500 hover:bg-red-50 rounded-xl text-sm font-medium transition">
                Выйти
              </button>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4">
        ...
        <section className="flex flex-col items-center pt-10 pb-8 sm:pt-12 sm:pb-10">
          ...
          <div className="flex items-center gap-5 mt-4">
            <button onClick={() => setShowFriendsListModal(true)} className="text-center group">
              <span className="block text-base font-semibold text-neutral-900 group-hover:text-primary-600 transition">{friendsCount}</span>
              <span className="text-xs text-neutral-400">друзей</span>
            </button>
            <div className="w-px h-8 bg-neutral-200" />
            <button onClick={() => setShowFollowersListModal(true)} className="text-center group">
              <span className="block text-base font-semibold text-neutral-900 group-hover:text-primary-600 transition">{followersCount}</span>
              <span className="text-xs text-neutral-400">подписчиков</span>
            </button>
          </div>
        </section>
        ...
        <section className="pb-10">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-lg font-semibold text-neutral-900">Вишлисты</h3>
            <button
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center py-20 text-center">
              ...
              <p className="text-neutral-500 text-sm mb-5">Создайте первый вишлист</p>
              <button
                onClick={() => setShowCreateModal(true)}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {wishlists.map(wishlist => {
                const daysLeft = wishlist.event_date
                  ...
                  : null
                return (
                  <div key={wishlist.id} className="group relative bg-white/60 backdrop-blur-sm rounded-2xl border border-neutral-200/40 hover:border-neutral-200/70 hover:shadow-md transition-all overflow-visible">
